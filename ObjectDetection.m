function [ I2 ] = ObjectDetection(Picture,FilenameHaarcasade,Options)
...
%if(nargout==0)
    ...
    %This part crop the image to display only the Face region.
    
    if(~isempty(Objects));

    %A= mean(Objects);

    %I2 = imcrop(Picture,[A(1) A(2) A(3) A(4)]);
    I2 = imcrop(Picture,[Objects(1,1) Objects(1,2) Objects(1,3) Objects(1,4)]);
    %imshow(I2)
    else
        I2 = Picture;
    end
